...
public class SocialForce : MonoBehaviour {
	...
	private int As = 2000;
	//private int Aw = 2000;
	private float Bs = 0.2f;
	//private double Bw = 0.4;
	private float M = 70.0f; //kg
	private float speed = 0.6f; // m/s
	private float responseTime=0.1f; // ss
	//private double wallDist_t=0.5; // m

	private float periodicX=-7.4f;
...
*/
		foreach (Rigidbody i in people) {
			if (i.name != "VRPerson") {
				...
				float theta;
				theta = Random.Range (-3.142f/ 7f, 3.142f / 10f);
